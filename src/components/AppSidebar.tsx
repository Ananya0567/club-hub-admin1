    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="border-b border-sidebar-border px-3 py-4">
        <div className="flex items-center gap-2">
          ...
          {!collapsed && (
            <div>
              <p className="text-sm font-bold">College Clubs</p>
              <p className="text-xs opacity-70">{panelLabel}</p>
            </div>
          )}
        </div>
      </SidebarHeader>
      ...
      <SidebarFooter className="border-t p-3 space-y-2">
        <Button
          ...
          className={collapsed ? "h-8 w-8 mx-auto" : "w-full justify-start gap-2"}
          onClick={toggleTheme}
        >
          {resolvedTheme === "dark" ? (
